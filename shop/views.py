...
def index(request):
    ...
    bilwa = {'allProds': allProds}
    return render(request, 'shop/index.html', bilwa)
...
def search(request):
    ...
    for cat in cats:
        ...
        if len(prod) != 0:
            allProds.append([prod, range(1, nSlides), nSlides])
    bilwa = {'allProds': allProds, "msg": ""}
    if len(allProds) == 0 or len(query) < 3:
        bilwa = {'msg': "No item available. Please make sure to enter relevant search query"}
    return render(request, 'shop/search.html', bilwa)
...
def checkout(request):
    if request.method == "POST":
        ...
        if 'onlinePay' in request.POST:
            ...
            bilwa_dict['CHECKSUMHASH'] = Checksum.generate_checksum(bilwa_dict, MERCHANT_KEY)
            return render(request, 'shop/paytm.html', {'bilwa_dict': bilwa_dict})
